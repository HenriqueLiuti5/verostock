    ...
    <div className="relative w-full">
      <header className="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
        <div>
          ...
          <p className="text-color5/60 dark:text-color4/60 mt-1 text-sm">Gerencie o catálogo e a quantidade de produtos.</p>
        </div>
      </header>
      ...
      <form onSubmit={handleSubmit} className="mb-8 bg-slate-50/50 dark:bg-slate-800/30 border border-color4/80 dark:border-slate-700/80 p-4 md:p-5 rounded-2xl shadow-sm grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 transition-colors duration-300">
        <div className="flex flex-col gap-1.5 md:col-span-2">
          <label className="text-[10px] font-bold text-color5/60 dark:text-color4/60 uppercase tracking-widest">Nome do Produto</label>
          <input 
        ...
        <div className="flex flex-col gap-1.5">
          <label className="text-[10px] font-bold text-color5/60 dark:text-color4/60 uppercase tracking-widest">Categoria</label>
          <select 
        ...
        <div className="flex flex-col gap-1.5">
          <label className="text-[10px] font-bold text-color5/60 dark:text-color4/60 uppercase tracking-widest">Qtd</label>
          <input 
            type="number" 
            min="0"
            placeholder="0" 
            className="w-full px-3 py-2 bg-white dark:bg-slate-800 border border-color4 dark:border-slate-700 rounded-xl text-color5 dark:text-white focus:outline-none focus:border-color2 focus:ring-2 focus:ring-color2/20 transition-all shadow-sm text-sm" 
            value={form.quantity} 
            onChange={e => {
        ...
        <div className="flex flex-col gap-1.5 md:col-span-3">
          <label className="text-[10px] font-bold text-color5/60 dark:text-color4/60 uppercase tracking-widest">Observações</label>
          <textarea 
            placeholder="Notas internas (opcional)" 
            className="w-full px-3 py-2 bg-white dark:bg-slate-800 border border-color4 dark:border-slate-700 rounded-xl text-color5 dark:text-white placeholder-color5/40 dark:placeholder-white/40 focus:outline-none focus:border-color2 focus:ring-2 focus:ring-color2/20 transition-all resize-none h-[50px] min-h-[50px] shadow-sm text-sm" 
          ...
          <table className="w-full text-left border-collapse whitespace-nowrap min-w-max">
            <thead>
              <tr className="bg-color1/50 dark:bg-slate-800/50 border-b border-color4 dark:border-slate-800">
                <th className="px-4 md:px-6 py-3.5 text-[10px] font-bold text-color5/60 dark:text-color4/60 uppercase tracking-widest">Nome</th>
                <th className="px-4 md:px-6 py-3.5 text-[10px] font-bold text-color5/60 dark:text-color4/60 uppercase tracking-widest">Categoria</th>
                <th className="px-4 md:px-6 py-3.5 text-[10px] font-bold text-color5/60 dark:text-color4/60 uppercase tracking-widest">Qtd</th>
                <th className="px-4 md:px-6 py-3.5 text-[10px] font-bold text-color5/60 dark:text-color4/60 uppercase tracking-widest max-w-[200px]">Descrição</th>
                <th className="px-4 md:px-6 py-3.5 text-[10px] font-bold text-color5/60 dark:text-color4/60 uppercase tracking-widest max-w-[200px]">Observações</th>
                <th className="px-4 md:px-6 py-3.5 text-[10px] font-bold text-color5/60 dark:text-color4/60 uppercase tracking-widest">Criado em</th>
            ...
            <tbody className="divide-y divide-color4/40 dark:divide-slate-800/80">
              {filteredProducts.map(p => (
                <tr key={p.id} className="hover:bg-color1 dark:hover:bg-slate-800/50 transition-colors duration-150 group">
                  <td className="px-4 md:px-6 py-3.5 text-sm font-medium text-color5 dark:text-white">{p.name}</td>
                  <td className="px-4 md:px-6 py-3.5 text-sm">
                    <span className="px-2.5 py-1 rounded-md bg-white dark:bg-slate-800 border border-color4 dark:border-slate-700 text-color5/80 dark:text-color4/80 text-[10px] font-bold tracking-wide uppercase shadow-sm">{getCategoryName(p.category)}</span>
                  </td>
                  <td className="px-4 md:px-6 py-3.5 text-sm font-mono font-medium text-color5 dark:text-white">
                    <span className={`px-2.5 py-1 rounded-md ${p.quantity <= 5 ? 'bg-red-50 dark:bg-red-500/10 text-red-600 dark:text-red-400 border border-red-200 dark:border-red-500/20' : ''}`}>{p.quantity}</span>
                  </td>
                  <td className="px-4 md:px-6 py-3.5 text-sm text-color5/60 dark:text-color4/60 max-w-[200px] truncate" title={p.description}>{p.description || '-'}</td>
                  <td className="px-4 md:px-6 py-3.5 text-sm text-color5/60 dark:text-color4/60 max-w-[200px] truncate" title={p.observations}>{p.observations || '-'}</td>
                  <td className="px-4 md:px-6 py-3.5 text-sm text-color5/80 dark:text-color4/80">{new Date(p.created_at).toLocaleDateString('pt-BR')}</td>
