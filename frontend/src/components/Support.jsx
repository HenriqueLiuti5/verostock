              ...
              {filteredTickets.map(t => (
                <tr key={t.id} className="hover:bg-color1 dark:hover:bg-slate-800/50 transition-colors duration-150 group">
                  ...
                  <td className="px-4 md:px-6 py-4 text-sm text-right">
                    ...
                    <button type="button" onClick={() => setTicketToDelete(t.id)} className="text-color5/40 dark:text-color4/40 hover:text-red-500 dark:hover:text-red-400 font-medium transition-colors md:opacity-0 md:group-hover:opacity-100 focus:opacity-100">
                      Excluir
                    </button>
                  </td>
                </tr>
      ...
      {ticketToDelete && (
        <div className="fixed inset-0 bg-color5/20 dark:bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-in fade-in duration-200">
          <div className="bg-white dark:bg-slate-900 border border-color4 dark:border-slate-800 p-6 sm:p-8 rounded-2xl shadow-xl w-full max-w-md scale-100 animate-in zoom-in-95 duration-200">
            ...
            <p className="text-color5/60 dark:text-color4/60 text-sm leading-relaxed mb-8">Tem certeza que deseja excluir este chamado de suporte? Esta ação não pode ser desfeita.</p>
            <div className="flex flex-col-reverse sm:flex-row justify-end gap-3">
              <button onClick={() => setTicketToDelete(null)} className="w-full sm:w-auto px-5 py-2.5 bg-white dark:bg-slate-800 border border-color4 dark:border-slate-700 text-color5 dark:text-white rounded-xl hover:bg-color1 dark:hover:bg-slate-700 font-medium transition-all duration-200 text-sm shadow-sm">
                Cancelar
              </button>
