...
export default function Support() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setErrorMessage(error.response?.data ? JSON.stringify(error.response.data) : "Erro ao salvar o chamado.");
    }
  };
  ...
  const resetForm = () => {
    setEditingId(null);
    setForm({ product_name: '', serial_number: '', client: '', problem_description: '', fix_action: '', status: 'PENDENTE' });
  };

  const filteredTickets = tickets.filter(t => {
    const searchLower = searchTerm.toLowerCase();
    const tDate = new Date(t.support_at);
    const start = dateStart ? new Date(dateStart + 'T00:00:00') : null;
    const end = dateEnd ? new Date(dateEnd + 'T23:59:59') : null;

    const matchesSearch = t.product_name.toLowerCase().includes(searchLower) ||
  ...
  const handleExportCSV = () => {
    const headers = ['ID', 'Produto', 'Serial', 'Cliente', 'Problema', 'Correção', 'Status', 'Data'];
    ...
    const csvContent = [
      ...
      ...filteredTickets.map(t => [
        ...
        escapeCsv(t.client),
        escapeCsv(t.problem_description),
        escapeCsv(t.fix_action),
        escapeCsv(t.status),
        new Date(t.support_at).toLocaleDateString('pt-BR')
      ].join(';'))
    ].join('\n');
            ...
            <tbody className="divide-y divide-color4/40 dark:divide-slate-800/80">
              {filteredTickets.map(t => (
                <tr key={t.id} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/50 transition-colors duration-150 group">
                  ...
                  <td className="px-4 md:px-6 py-3.5 text-sm text-color5/80 dark:text-slate-300">{t.client || '-'}</td>
        ...
        <div className="fixed inset-0 bg-color5/20 dark:bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-in fade-in duration-200">
          <div className="bg-white dark:bg-slate-900 border border-color4/80 dark:border-slate-800 p-6 sm:p-8 rounded-2xl shadow-xl w-full max-w-md scale-100 animate-in zoom-in-95 duration-200">
            <div className="flex items-center gap-3 mb-2">
              ...
              <h3 className="text-xl font-bold text-red-600 dark:text-red-400 tracking-tight">Aviso</h3>
            </div>
